...
//Fraction, credit: copied and modified from https://github.com/JadenGeller/Fractional/blob/master/Sources/Fractional.swift
public typealias Fraction = Fractional<Int>

private func gcd<Number: IntegerType>( lhs: Number, _ rhs: Number) -> Number {
    var resultRhs = rhs
    var resultLhs = lhs
    while resultRhs != 0 { (resultLhs, resultRhs) = (resultRhs, resultLhs % resultRhs) }
    return resultLhs
}
...
public func reduce<Number: IntegerType>(numerator numerator: Number, denominator: Number) -> (numerator: Number, denominator: Number) {
